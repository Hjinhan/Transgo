...
bool Encode(const State* state, float* encode_state) {
    ...
    return true;
     ...
     // 将特征切成块
void getSubEncode(int *encode_state, int *sub_ecode_state,int sub_board_size, int encode_state_channels, int cut_num)
   {
       get_sub_encode(encode_state, sub_ecode_state, sub_board_size, encode_state_channels, cut_num);
   }
