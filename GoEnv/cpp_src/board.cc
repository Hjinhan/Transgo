...
void get_sub_encode(int *encode_state, int *sub_ecode_state,int sub_board_size, int encode_state_channels, int cut_num)    // 支持切4或5块
	{
		 
		int *c_p[20];  // 指针数组
		int a;
		int pos, pos1, interval;
		interval = GoComm::BOARD_SIZE - sub_board_size;
		for (int ix = 0; ix < encode_state_channels; ix++) c_p[ix] = (int *)&encode_state[ix * GoComm::MAX_COORD];// encode_state 每个特征设置一个地址
		...
		for (int i = 0; i < cut_num; i++)
		{
			...
			if (i == 0)  // 左上
			{   
				for (int j = 0; j < encode_state_channels; j++)
				{
					for (a = 0; a < GoComm::MAX_COORD; a++)
					{
						pos = a;   
                        int pos_x = COORD_TO_X(pos), pos_y = COORD_TO_Y(pos);
						if (pos_x < sub_board_size && pos_y < sub_board_size)
						{
							//cout << "333:  " << c_p[j][a] << "\n";
							pos1 = pos_y *sub_board_size + pos_x;
							sub_ecode_state[i*sub_board_size*sub_board_size*encode_state_channels + sub_board_size*sub_board_size*j+ pos1] = c_p[j][a];
						}
					}
				}
			}
			...
			if (i == 1) // 右上
			{
				for (int j = 0; j < encode_state_channels; j++)
				{
					for (a = 0; a < GoComm::MAX_COORD; a++)
					{
						pos = a;
                        int pos_x = COORD_TO_X(pos), pos_y = COORD_TO_Y(pos);
						//if ((interval <= x < board_size) && (0 <=y < sub_board_size))  //   x =0,y=1 也会满足条件
						if (pos_x >= interval && pos_x < GoComm::BOARD_SIZE && pos_y >= 0 && pos_y < sub_board_size)
						{
							pos1 =pos_y *sub_board_size + pos_x - interval;
							sub_ecode_state[i*sub_board_size*sub_board_size*encode_state_channels + sub_board_size*sub_board_size*j + pos1] = c_p[j][a];
						}
					}
				}
			...
			if (i == 2) // 左下
			{
				for (int j = 0; j < encode_state_channels; j++)
				{
					for (a = 0; a < GoComm::MAX_COORD; a++)
					{
						pos = a;
                        int pos_x = COORD_TO_X(pos), pos_y = COORD_TO_Y(pos);
						//if (0 <= x < sub_board_size && interval <=y < board_size)
						if (pos_x >= 0 && pos_x < sub_board_size && pos_y >= interval && pos_y < GoComm::BOARD_SIZE)
						{
							pos1 = ( pos_y - interval) *sub_board_size + pos_x;
							sub_ecode_state[i*sub_board_size*sub_board_size*encode_state_channels + sub_board_size*sub_board_size*j + pos1] = c_p[j][a];
						}
					}
				}
			...
			if (i == 3) //右下
			{
				for (int j = 0; j < encode_state_channels; j++)
				{
					for (a = 0; a < GoComm::MAX_COORD; a++)
					{
						pos = a;
                        int pos_x = COORD_TO_X(pos), pos_y = COORD_TO_Y(pos);
						//if (interval <= x < board_size && interval <=y < board_size)
						if (pos_x >= interval && pos_x < GoComm::BOARD_SIZE && pos_y >= interval && pos_y < GoComm::BOARD_SIZE)
						{
							pos1 = (pos_y - interval) *sub_board_size + pos_x - interval;
							sub_ecode_state[i*sub_board_size*sub_board_size*encode_state_channels + sub_board_size*sub_board_size*j + pos1] = c_p[j][a];
						}
					}
				}
			...
			if (i == 4) // 中心
			{
				...
				for (int j = 0; j < encode_state_channels; j++)
				{
					for (a = 0; a < GoComm::MAX_COORD; a++)
					{
						pos = a;
                        int pos_x = COORD_TO_X(pos), pos_y = COORD_TO_Y(pos);
						//if (ter <= x < board_size - ter && ter <=y < board_size - ter)
						if ( pos_x >= ter && pos_x < GoComm::BOARD_SIZE - ter && pos_y >= ter && pos_y < GoComm::BOARD_SIZE - ter)
						{
							pos1 = (pos_y - ter) *sub_board_size + pos_x - ter;
							sub_ecode_state[i*sub_board_size*sub_board_size*encode_state_channels + sub_board_size*sub_board_size*j + pos1] = c_p[j][a];
						}
					}
				}
