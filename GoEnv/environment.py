...
class GoEnv:
    def __init__(self,config):
        ...
        self.c_getSubEncode = self.lib.getSubEncode  #  切割特征
        self.c_getSubEncode.argtypes = [ndpointer(c_float), ndpointer(c_float), c_int, c_int, c_int]
        self.c_getSubEncode.restype = None
    ...
    def subEncode(self,encode):
          sub_encode = np.zeros([4,self.encoded_dim, self.sub_board_size, self.sub_board_size], dtype="float32")
          self.c_getSubEncode(encode, sub_encode, self.sub_board_size, self.encoded_dim, 4)
          return sub_encode
    ...
    def location_to_action(self,location):
        row = location[0] 
        col = location[1]
        return self.config.board_size * row +col
    ...
    def encode_grid(self,state):   # debug
        encode = self.encode(state)
        sub_encode = self.subEncode(encode)

        grid_encode = np.zeros([1, self.board_size, self.board_size])
        grid_encode += np.sum(encode[0:3],0)
        grid_encode -= np.sum(encode[3:6],0)
        print("grid_encode:\n", grid_encode)

        grid_sub_encode = np.zeros([4, self.sub_board_size, self.sub_board_size])

        grid_sub_encode[0] += np.sum(sub_encode[0,0:3],0)
        grid_sub_encode[0] -= np.sum(sub_encode[0,3:6],0)

        grid_sub_encode[1] += np.sum(sub_encode[1,0:3],0)
        grid_sub_encode[1] -= np.sum(sub_encode[1,3:6],0)

        grid_sub_encode[2] += np.sum(sub_encode[2,0:3],0)
        grid_sub_encode[2] -= np.sum(sub_encode[2,3:6],0)

        grid_sub_encode[3] += np.sum(sub_encode[3,0:3],0)
        grid_sub_encode[3] -= np.sum(sub_encode[3,3:6],0)

        print("grid_sub_encode:\n", grid_sub_encode)
