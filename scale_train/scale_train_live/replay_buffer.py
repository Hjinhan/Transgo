...
class ReplayMemory_Random():    # random
    ...
    def __init__(self,config):
      ...
      self.blank_trans = (np.zeros((config.encode_state_channels,config.local_board_size,config.local_board_size)), 
                                        np.zeros((config.local_board_size**2+1)), 0.0,np.zeros((config.local_board_size**2)))  
